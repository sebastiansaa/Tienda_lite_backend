...
import { UpdateItemDto } from '../../api/dtos/request';
...
describe('UpdateItemDto validation', () => {
    ...
    it('rechaza cantidad inválida o faltante', async () => {
        await expect(
            pipe.transform({}, { type: 'body', metatype: UpdateItemDto }),
        ).rejects.toThrow();
        await expect(
            pipe.transform({ quantity: 0 }, { type: 'body', metatype: UpdateItemDto }),
        ).rejects.toThrow();
    });

    it('rechaza campos extra', async () => {
        await expect(
            pipe.transform({ quantity: 1, extra: 'no' }, { type: 'body', metatype: UpdateItemDto }),
        ).rejects.toThrow();
    });
